...
9. [전체 작업 순서 (STEP 1~23)](#9-전체-작업-순서-step-123)
10. [작업 흐름도 (평면도)](#10-작업-흐름도-평면도)
11. [코디세이 API 연동 구조](#11-코디세이-api-연동-구조)
12. [로컬 실행 방법](#12-로컬-실행-방법)
13. [Vercel 배포 방법 및 환경 변수](#13-vercel-배포-방법-및-환경-변수)
14. [배포 중 발생한 오류와 해결](#14-배포-중-발생한-오류와-해결)
15. [AI(Claude) 사용 로그 요약](#15-aiclaude-사용-로그-요약)
16. [기능요구사항 대비 결과](#16-기능요구사항-대비-결과)
17. [자체 점검 체크리스트](#17-자체-점검-체크리스트)
18. [GitHub 업로드 확인](#18-github-업로드-확인)
...
**인사이트 요약 (Fact–Why–Action, 자세한 내용은 REPORT.md 6장 참고)**

1. **2026-03-03 급등(+3.17%)** — 이란-미국 전쟁(2026-02-28 발발) 여파로 안전자산 선호 심리가
   강해지며 원화 약세 압력이 발생했을 가능성. *(이 가설은 학습자가 먼저 제시했고, AI가 웹
   검색으로 실제 발발일·종전 협상 일정을 검증했습니다 — 15장 AI 사용 로그 참고)*
   **조치(Action):** 지정학적 리스크 이벤트 발생 시 환율 모니터링 주기를 단축하고,
   외화 자산·부채 보유 시 환헤지 비율을 재점검할 것을 권고합니다.
2. **2024-12-03 급등(+1.66%)** — 비상계엄 선포로 정치적 불확실성이 급격히 커지며 원화 약세
   압력 발생. *(이 가설은 AI가 먼저 제시했고, 학습자가 웹 검색으로 재검증했습니다)*
   **조치(Action):** 국내 정치적 불확실성이 급격히 커지는 시점에는 환전·해외송금 등을
   서두르기보다 며칠간 변동성 안정화 여부를 확인한 뒤 진행할 것을 권고합니다.
3. **2026년 7~9월 3개월 연속 하락** — 미·이란 종전 협상 종료, 연준 통화정책 전망 변화,
   수출기업 달러 매도 물량 증가 등 복합 요인으로 원화 강세(환율 하락) 국면 지속
   **조치(Action):** 한 방향 추세가 3개월 이상 이어질 경우 단기 되돌림 가능성과 추세를
   뒷받침하는 거시 요인(금리차·수출입 동향)의 변화 여부를 주기적으로 점검할 것을 권고합니다.

> 위 조치(Action)는 일반적인 리스크 모니터링 관점의 참고 권고이며, 특정 매매 시점을
> 지시하는 투자 조언이 아닙니다.
...
- 분석 기간(2024-07~2026-09) 동안 원/달러 환율은 하락→재상승→재하락의 물결 형태 흐름을 보였습니다.
- 레벨(원화 값) 기준으로는 통계적 이상치가 없었지만, 일간 변화율 기준으로는 명확한 급등락일이 존재했으며, 이들은 실제 국내외 정치·지정학적 이벤트와 시기적으로 맞아떨어졌습니다.
- 2026년 7~9월과 같이 특정 구간 전체가 한 방향으로 움직이는 추세도 관찰되었으며, 이는 단일 사건보다는 복합적인 거시경제 요인의 결과로 해석됩니다.

### 한계점

- 본 분석은 관찰된 시점과 외부 이벤트의 "시기적 일치"를 근거로 한 가설이며, 통계적으로 인과관계를 검증한 것은 아닙니다.
- 환율에 영향을 미치는 다른 변수(금리차, 수출입 지표, 유가 등)를 함께 분석하지 않아, 특정 사건의 영향력을 다른 요인과 분리해서 정량화하지는 못했습니다.
- 데이터 출처가 매매기준율(1일 1개 값)이라 장중 변동은 반영하지 못합니다.
- 위 인사이트의 조치(Action)는 일반적인 리스크 모니터링 관점의 참고 권고이며, 투자 조언이 아닙니다.

### AI 사전평가 반영 사항

제출 전 AI 사전평가(94%, 17개 항목 중 16개 통과)를 거쳤습니다. 유일한 FAIL 항목은 "인사이트의
Fact→Why→Action 중 Action(구체적 권장 조치) 명시 부족"이었고, 이를 반영해 위 인사이트 3개
모두에 조치(Action) 문장을 추가했습니다. 평가 상세 내역은 REPORT.md 7장과 결과보고서 Ⅶ장에
정리되어 있습니다.

---

## 9. 전체 작업 순서 (STEP 1~23)
...
### 11-1. 로컬 연동 테스트 — `codyssey_api.py`
...
### 11-2. 대시보드 연동 — `web/api/explain.js` (Vercel Serverless Function)
...
이력이 있습니다(39, 40번) — 14장에서 자세히 다룹니다.
...
자세한 내용은 [`REPORT.md`](./REPORT.md#10-ai-사용-로그)와 결과보고서 Ⅵ장을 참고하세요.
...
| 결론 및 한계점 | `REPORT.md` 7장,<br/>본 문서 8장 | ✅ |
| AI 사용 로그 작성 | `REPORT.md` 10장,<br/>본 문서 15장 | ✅ |
...
- [x] 인사이트 3개 이상, Fact-Why-Action 구조(조치까지 포함)
- [x] 결론 및 한계점 서술
...
- [x] 제출 전 AI 사전평가 반영(인사이트 Action 보완)
...
- `.env`는 `.gitignore`에 의해 커밋 이력에서 제외됨(아래 19장 참고)
...
- 구현 과정에서 발생한 오류와 해결 과정은 14장과 `REPORT.md` 9장(결과보고서 Ⅷ장)에 함께
  기록했습니다.
- AI(Claude)는 코드 작성·디버깅·인사이트 사실관계 검증(웹 검색)에 활용했으며, 최종 해석·결론
  문장은 직접 작성했습니다.
- 제출 전 AI 사전평가를 거쳐 인사이트에 조치(Action)를 보완한 이력은 8장과 `REPORT.md` 7장,
  결과보고서 Ⅶ장에 정리되어 있습니다.
